 ...
 * @author xinouch
 */
public class XLRule implements Rule
{
	 ...
	 */
	public XLRule (boolean isDeterminist)
	{
	...
	public boolean canBeApplied (int index, ListSymbols phraseFromPreviousIteration)
	{
		...
		if (phraseFromPreviousIteration.get(index).getCharacter() != preExpr.getCharacter())
			return false;

		// validation des trucs avant
		while ((i < index) && (j < sizePrev))
		{
	...
	public int applyOnce (int index, ListSymbols phraseFromPreviousIteration, ListSymbols toBeModified)
	{
		...
		if (!canBeApplied(index, phraseFromPreviousIteration))
			return 0;
