...
import parser.ILRule;
import parser.OLRule;
import parser.Rule;
import parser.Symbol;


/**
 * classe permettant l'analyse d'un fichier contenant des grammaires. Peut aussi être utilisée pour analyser des règles écrites dans le
 * programme. les grammaires du fichier sont stockées dans leur ordre d'appartition dans une liste de grammaires: Analyzer.getGrammars().
 * 
 * @author xinouch
 */
public class Analyzer implements AnalyzerConstants
{
		...
		label_16: while (true)
		{
			...
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk)
			{
				case PREVIOUS:
					...
					{
						if (true)
							throw new BadFileException("In rules definition:\u005cn"
			...
			if (current.getType() == Grammar.TYPE_DOL || current.getType() == Grammar.TYPE_DIL)
			{
				...
				for (int i = 0; i < size; i++)
				{
					...
					{
						if (true)
							throw new BadFileException("In rules definition:\u005cn" + rule.getPreExpr().getCharacter()
									+ " appear more than once in the left part of a rule " + "declaration while the grammar is determinist");
					}
				}
			}
	...
	{
		jj_la1_0 =
				new int[] { 0x2000000, 0x0, 0x2000000, 0x2000000, 0xf0000, 0x2000000, 0x2000000, 0x2000000, 0x2000000, 0xffc0, 0x8000000,
						0x2000000, 0x0, 0x2000000, 0xc, 0x2000000, 0x2000000, 0x20, 0x2000000, 0x2000000, 0x0, 0x80000000, 0x0, 0x0, 0x0,
						0x8000000, 0x2000000, 0x0, 0x2000000, };
	}

	private static void jj_la1_init_1 ()
	{
		jj_la1_1 =
				new int[] { 0x0, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xc0,
						0x0, 0xc0, 0x1, 0xc2, 0x0, 0x0, 0x40, 0x0, };
	}
	...
	/*
	 * public static void main (String[] args) throws ParseException, NumberFormatException, BadFileException { Analyzer analyz = new
	 * Analyzer(System.in, "UTF-8"); analyz.validationStart(); for (int i = 0; i < analyz.getGrammars().size(); i++)
